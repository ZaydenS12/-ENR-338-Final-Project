...
#define NX 300     // Grid points in X
#define NY 200    // Grid points in Y
#define NIT 50.0   // Pressure iterations per time step
#define NT 500.0   // Total time steps
#define T 0.05    // Actual time in seconds
#define SKIP 4   // How often to write data
#define VI 1.5 // The starting speed of the pipe
#define XI 20 // Starting X coord for the box
#define XF 25 // Ending X coord for the box
#define YI 70
#define YF 130
#define scalex 4.0
#define scaley 2.0
...
double lx = 30; // physical length in X direction
double ly = 20; // physical length in Y direction
double rho = 1.0;
double nu = .3; // extremely viscous 
double dt = 1.0*T/NT;
double X;
double Y;
// Arrays
 ...
 fprintf(stderr, "Starting Simulation...\n");
  ...
  for (int n=0; n <= NT; n++) {
    ...
    for (int it = 0; it < NIT; it++) {
      ...
      for (int j=0; j < NY; j++) {
        p[j][0] = p[j][1];      // Left 
        p[j][NX-1] = 0;  // Right set to zero (open pipe)
      }
      for (int i=0; i < NX; i++) {
        p[0][i] = p[1][i];      // Bottom
    ...
    // Stationary Walls
			...
			for (int j=0; j < NY; j++) {
			u[j][NX-1] = u[j][NX-2];// X, right wall 
			v[j][NX-1] = v[j][NX-2];//Y, right wall 
			//p[j][NX] = 0;	
			//pn[j][NX] = 0;		
      u[j][0] = VI; // X , Left wall   
      v[j][0] = 0; // Y, left wall
    }
    for (int i=0; i < NY; i++) {
      u[0][i] = u[1][i]; //Bottom X velocity
			v[0][i] = v[1][i]; //Bottom y velocity 


      u[NY-1][i] = u[NY-2][i];//top wall x velocity  
      v[NY-1][i] = v[NY-2][i]; //top wall y velocity 
    ...
    //BLOCKER
		...
		for (int i=YI; i < YF ; i++){ //front vertical portion
			u[i][XI] = 0;
			v[i][XI] = v[i][XI-1];
		}	
	
		for (int i=YI; i < YF; i++){ //rear vertical portion
			u[i][XF] = 0;
			v[i][XF] = v[i][XI+1];
		}	
