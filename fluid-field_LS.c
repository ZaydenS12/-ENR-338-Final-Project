...
#define XI (NX/4) // Starting X coord for the box	//Tweaked by Louis that box starts 25% into the pipe
#define Obstacle_Width (16)				//Tweaked by Louis for a narrow box
#define XF (XI + Obstacle_Width) // Ending X coord for the box //Tweaked by Louis to ensure box starts at a spot and has controllable width
...
int main() {
  ...
  for (int n=0; n <= NT; n++) {
    ...
    for (int it = 0; it < NIT; it++) {
      ...
      for (int j=0; j < NY; j++) {
        p[j][0] = p[j][1];      // Left
        p[j][NX-1] = 0;  // Right (zero pressure outlet)
      }
      for (int i=0; i < NX; i++) {
        p[0][i] = p[1][i];      // Bottom
    ...
    // Stationary Walls
		...
		for (int j=0; j < NY; j++) {
      u[j][0] = VI; u[j][NX-1] = u[j][NX-2];//left wall ; right wall (x velocity) //Louis adjusted. Vital
      v[j][0] = 0; v[j][NX-1] = v[j][NX-2];//left wall : right wall (y velocity) //NOTE: DNE 0, it equals the place before it!!! //Louis adjusted, vital
    }
    for (int i=0; i < NX; i++) {
      u[0][i] = 0; v[0][i] = 0; //bottom x velocity; bottom y velocity 
      u[NX-1][i] = 0; v[NX-1][i] = 0; //bottom x velocity; bottom y velocity 
    ...
    //BLOCKER
		// Set the inside to 0
		for (int i = ((NY/3)+1); i< ((2*(NY/3))-1); i++){	//Louis Note dimensions changed so box is 1/3 width of pipe
		...
		for (int i=NY/3; i < (2*NY/3); i++){ //front vertical portion	\\Louis Note dimension changed so box is 1/3 width of pipe
			u[i][XI] = 0;		
			v[i][XI] = v[i][XI-1];
		}	
	
		for (int i=NY/3; i < (2*NY/3); i++){ //rear vertical portion   \\LOUIS NOTE dimenions changed so box is 1/3 width of pipe
			u[i][XF] = 0;
			v[i][XF] = v[i][XF-1];
		}	
			
		for (int i=XI; i < XF; i++){ //Top portion
			u[2*NY/3][i] = u[2*NY/3][i-1];
			v[2*NY/3][i] = 0;
		}	
