...
int main() {
  ...
  for (int n=0; n <= NT; n++) {
    ...
    // Stationary Walls
			...
			for (int j=0; j < NY; j++) {
      u[j][0] = 2; u[j][NX-1] = 2;//left wall ; right wall (x velocity) 
      v[j][0] = 0; v[j][NX-1] = 0;//left wall : right wall (y velocity)
    }
    for (int i=0; i < NX; i++) {
      u[0][i] = 0; v[0][i] = 0; //bottom 
		
      u[NY-1][i] = 2;//top wall x velocity  
      v[NY-1][i] = 0; //top wall y velocity 

      u[0][i] = 2;//top wall x velocity  
